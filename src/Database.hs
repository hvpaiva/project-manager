...
getBudget _ = do
    ...
    pure Budget { budgetIncome = income, budgetExpediture = expenditure }
  
getTransactions :: ProjectId -> IO [Transaction]
getTransactions _ = do
    sale <- Sale . Money <$>  getStdRandom (randomR (0, 1000))
    purchase <- Purchase . Money <$> getStdRandom (randomR (0, 10000))
    pure [sale, purchase]
